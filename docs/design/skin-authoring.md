...
under `internal/tui/theme/skins/` shipped in the binary. For the
schema and loader reference (writing a user-side skin under
`<config-dir>/a10r/skins/`), see
[`k9s-skins-dropin.md`](./k9s-skins-dropin.md). For the policy that
permits in-tree-authored skins alongside upstream-synced ones, see
...
1. Pick a name. Bundled-skin names use `<family>-<mode>` with an
   optional `-transparent` suffix (e.g., `ovhcloud-dark`,
   `ovhcloud-dark-transparent`). The name must match the file
...
```yaml
# ovhcloud-dark — a10r skin inspired by the OVHcloud Design System
# (https://github.com/ovh/design-system, Apache-2.0).
#
# a10r is an independent open-source project and is not affiliated
# with, endorsed by, or supported by OVHcloud or OVH SAS. The
# "OVHcloud" name is used nominatively to identify the visual
# inspiration.
```
...
(see the role map in
[`k9s-skins-dropin.md`](./k9s-skins-dropin.md#role-mapping-full)).
The bundled `ovhcloud-*` family uses these anchors:

| Skin | `body.bgColor` | `body.fgColor` |
|---|---|---|
| `ovhcloud-dark` | `#1a1a1a` (`ods-palette-gray-900`) | `#f2f2f2` (`ods-palette-gray-050`) |
| `ovhcloud-light` | `#ffffff` (`ods-palette-white`) | `#4d5592` (`ods-palette-twilight`, the `ods-color-text` token) |
| `ovhcloud-dark-transparent` | `default` (terminal-native) | `#f2f2f2` |
| `ovhcloud-light-transparent` | `default` | `#4d5592` |
...
ODS-style semantic scales (`primary`, `information`, `success`,
...
authored:
  - palette_source:
      repo: https://github.com/ovh/design-system
      commit: <pinned-SHA-at-authoring-time>
      license: Apache-2.0
      file: packages/themes/src/default/_variables.scss
    files:
      - ovhcloud-dark.yaml
      - ovhcloud-dark-transparent.yaml
      - ovhcloud-light.yaml
      - ovhcloud-light-transparent.yaml
```
