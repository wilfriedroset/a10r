...
- **Tenant prompt on `s` from a multi-tenant scope with no clear cursor row** (e.g. user is on the alerts list with `<0> all` selected and no row focused). Today the rule is "first in-scope tenant alphabetically" — that's deterministic and good enough. A future modal could explicitly ask.
