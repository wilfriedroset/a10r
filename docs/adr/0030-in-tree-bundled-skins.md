...
`ovhcloud-*` skins added alongside the eight `catppuccin-*` files do
not fit that reading. This ADR records the explicit policy: the
...
proves this is worth shipping. ODS does not publish k9s-format
skins, so mirroring an upstream is not an option for an OVHcloud
skin family — we either author in-tree or do without. Authoring
...
ambiguous. Drift between ODS upstream and the recorded
`palette_source.commit` is silent today; a `skins doctor` lint
that walks `authored:` and warns on stale pins can land later if
the maintenance load justifies it.

Considered and rejected: (a) **user-side only** (ship `ovhcloud-*`
as files under `examples/` for users to copy into
`<config-dir>/a10r/skins/`) — the four skins become second-class
citizens, discoverable only via the README, and `--theme
ovhcloud-dark` does not work out of the box; the discoverability
