...
func TestLabelPredicate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unquoted comma in regex value falls back to text", input: "cluster_id=~(a,b)", ok: false},
		{name: "quoted comma in regex value stays one matcher", input: `cluster_id=~"(a,b)"`, ok: true, labels: map[string]string{"cluster_id": "a,b"}, matches: true},
		{name: "quoted-comma matcher ANDs with another", input: `cluster_id=~"(a,b)",role=consul`, ok: true, labels: map[string]string{"cluster_id": "a,b", "role": "consul"}, matches: true},
		{name: "escaped quote in quoted value does not split", input: `cluster_id=~"a\"b,c"`, ok: true, labels: map[string]string{"cluster_id": `a"b,c`}, matches: true},
		{name: "&& inside quotes is literal, not a separator", input: `cluster_id=~"a&&b"`, ok: true, labels: map[string]string{"cluster_id": "a&&b"}, matches: true},
		{name: "unbalanced quote falls back to text", input: `cluster_id=~"(a,b`, ok: false},
