...
func TestNew_SeedInstancesRenderWithDistinguishingLabelsInstanceFirst(t *testing.T) {
	...
	p := newPage(t,
		instance("fp-1", "warning", backend.AlertStateActive, map[string]string{"instance": "web-1", "pod": "p1"}),
		instance("fp-2", "warning", backend.AlertStateActive, map[string]string{"instance": "web-2", "pod": "p2"}),
	)
...
func TestCommonStrip_ShownByDefaultHiddenAfterShiftC(t *testing.T) {
	...
	p := newPage(t,
		...
		instance("fp-2", "warning", backend.AlertStateActive, map[string]string{"instance": "web-2"}),
	)
	out := testutil.StripStyle(p.View(120, 20))
...
func TestCommonStrip_OmittedWhenNoSharedLabelsBeyondAlertname(t *testing.T) {
	...
	a := instance("fp-1", "critical", backend.AlertStateActive, map[string]string{"instance": "web-1"})
	a.Labels["cluster"] = "a"
	b := instance("fp-2", "warning", backend.AlertStateActive, map[string]string{"instance": "web-2"})
	b.Labels["cluster"] = "b"
	p := newPage(t, a, b)
	out := testutil.StripStyle(p.View(120, 20))
	require.NotContains(t, out, "common:",
...
func TestPoll_IgnoresAlertsForOtherAlertnames(t *testing.T) {
	...
	mine := instance("fp-mine", "warning", backend.AlertStateActive, map[string]string{"instance": "web-1"})
...
func TestFilter_NarrowsView(t *testing.T) {
	...
	p := newPage(t,
		instance("fp-1", "warning", backend.AlertStateActive, map[string]string{"instance": "web-1"}),
		instance("fp-2", "warning", backend.AlertStateActive, map[string]string{"instance": "db-1"}),
	)
...
func TestSort_FingerprintTieBreakIsDeterministic(t *testing.T) {
	...
	mk := func(fp string) backend.Alert {
		return instance(fp, "warning", backend.AlertStateActive, map[string]string{"instance": "same"})
	}
...
func TestSort_SeverityDescThenFingerprint(t *testing.T) {
	t.Parallel()
	p := newPage(t,
		instance("fp-w", "warning", backend.AlertStateActive, map[string]string{"instance": "w"}),
		instance("fp-c2", "critical", backend.AlertStateActive, map[string]string{"instance": "c2"}),
		instance("fp-c1", "critical", backend.AlertStateActive, map[string]string{"instance": "c1"}),
	)
...
func TestS_UnionPushesSilencesWithRestrictIDs(t *testing.T) {
	t.Parallel()
	a := instance("fp-1", "warning", backend.AlertStateSuppressed, map[string]string{"instance": "web-1"})
	a.SilencedBy = []string{"sil-1", "sil-2"}
	b := instance("fp-2", "warning", backend.AlertStateSuppressed, map[string]string{"instance": "web-2"})
	b.SilencedBy = []string{"sil-2", "sil-3"}
	p := newPage(t, a, b)

	require.Equal(t, []string{"sil-1", "sil-2", "sil-3"}, p.silencedByUnion(),
...
func TestState_FullVsCompactTokenRendering(t *testing.T) {
	...
	p := newPage(t,
		instance("fp-1", "warning", backend.AlertStateSuppressed, map[string]string{"instance": "web-1"}),
	)
	out := testutil.StripStyle(p.View(120, 20))
...
func TestReadOnly_SilenceKeyFlashesHint(t *testing.T) {
	...
	p := New(Options{
		...
		AlertName: "HighLatency",
		ReadOnly:  true,
		Instances: []backend.Alert{instance("fp-1", "warning", backend.AlertStateActive, map[string]string{"instance": "web-1"})},
	})
