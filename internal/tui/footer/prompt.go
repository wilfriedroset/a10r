...
// emoji + `>` chevron; a10r keeps the shape but swaps the dogs for
// its own marks — 🫡 (the project's identity emoji) for command,
// 🔬 (search/inspect) for filter — so the two modes stay distinct
// while the chevron cue carries over for users coming from k9s.
