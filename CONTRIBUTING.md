...
follow-up PRs. Every commit is bisect-safe from v0.1.0 onward -- the
v0.1.0 tag is the squash point, and the pre-v0.1.0 launch history is a
narrative of ten phase-commits that are not individually buildable.
...
- More themes. The bundled set is the catppuccin family; user skins
  under `<config-dir>/skins/` shadow them by basename.
