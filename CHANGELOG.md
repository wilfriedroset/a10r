...
- **Vanilla Alertmanager v2 client** — the `Client` interface,
  sentinel errors, HTTP transport with read and write paths
  (alerts, silences, status, receivers, groups), capability
  flags, and a retry policy.
...
- **Pages** — alerts list and detail, silences list and form,
  ...
  built on shared list-page and detail-page bases.
- **Polish, command surface, and doctor** — tenant picker and
  ...
  overlay, external-editor handoff, bracketed paste, watch-mode
  toggles, init wizard, `list` subcommands with json / yaml /
  table output, and the `doctor` preflight checks.
- **Bundled skins** — twelve skins across the catppuccin and
  ovhcloud families (each with a `-transparent` sibling) using a
  k9s drop-in schema; user skins under `<config-dir>/skins/`
  shadow bundled ones by basename.
- **Security hardening** — host-pinned auth and header round
  trippers, cross-origin redirect refusal, TLS warnings, a
  response body cap, audit logging on silenced writes, and a
  tempfile editor handoff.
